...
import { getResidentes, registrarBitacora } from '../../api/usuarios/usuarios'
...
export default function FaceRecognition() {
    ...
    const fetchVehiculos = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/unidadpertenencia/vehiculos`
        )
  ...
  const verificarPlacaLocal = (placaDetectada) => {
    ...
    return vehiculo
      ? { existe: true, propietario: vehiculo }
      : { existe: false }
  }
      ...
      try {
        ...
        if (detections.length) {
          ...
          if (match.label !== 'unknown') {
            ...
            await registrarBitacora({
              accion: `Acceso facial concedido: ${match.label}`
            })
          } else {
            setMensaje('Acceso facial denegado')
            await registrarBitacora({ accion: 'Acceso facial denegado' })
          }
        }
        ...
        if (placas.length) {
          for (const placa of placas) {
            ...
            if (resultado.existe) {
              setMensaje(
                `Placa autorizada: ${placa} - ${resultado.propietario.marca} ${resultado.propietario.modelo}`
              )
              await registrarBitacora({
                accion: `Placa autorizada: ${placa} - ${resultado.propietario.marca} ${resultado.propietario.modelo}`
              })
            } else {
              setMensaje(`Placa NO autorizada: ${placa}`)
              await registrarBitacora({
                accion: `Placa NO autorizada: ${placa}`
              })
            }
